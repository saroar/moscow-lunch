class OmniauthUser
  ...
  def find_or_create(auth, organization)
    if omniauth_user(auth, organization)
      ...
      create_omniauth_user(auth, organization)
    end
  end

  private

  def omniauth_user(auth, organization)
    User.where(provider: auth.provider, uid: auth.uid).first
  end

  def registered_user(auth, organization)
    User.where(email: auth.info.email).first
  end

  def create_omniauth_user(auth, organization)
    User.create(
